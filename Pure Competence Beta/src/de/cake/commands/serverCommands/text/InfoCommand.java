...
public class InfoCommand implements ServerCommand {
	...
	/**
	 * This could take a rework due to confusing Code... but it works now (mostly) :D.
	 * 
...
//		NullPointerException if USers Color is Default
		try {
			...
			builder.addField("Joined Server", m.getTimeJoined().getDayOfMonth() + "/" + m.getTimeJoined().getMonthValue() + "/" + m.getTimeJoined().getYear(), true);
			builder.addField("Display Color", "R: " + color.getRed() + " G: " + color.getGreen() + " B: " + color.getBlue(), true);
			builder.addField("Highest role", m.getRoles().get(0).getAsMention(), true);
			builder.addField("Activities", Activity.size() == 2 ? Activity.get(0).toString() + ", " + Activity.get(1).toString() : Activity.get(0).toString(), false);
