...
public class HelpCommand implements PrivateCommand {
		...
		try {
			...
			if(args.length == 1) {
				...
				if(args[1].equalsIgnoreCase("meme")) {
					...
					cmdH.addField("Example of usage", "$sendmsg <" + u.getIdLong() + "> <Title> This Title should be linked to YouTube <Title>\r\n" + 
					...
					cmdH.addField("Description", "$sendmsg <" + u.getIdLong() + "> <-- Defines the User to send the message to (Example is your own)\r\r\n" + 
				...
				if(i != 0) {
					...
					channel.sendMessage(cmdH.build());
				} else {
					builder.setTitle("**BotHelp** \r");
