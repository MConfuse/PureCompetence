...
public class SkipFwCommand implements ServerCommand {
	...
	public void performCommand(Member m, TextChannel channel, Message message) {
		...
		if((state = m.getVoiceState()) != null) {
			...
			if((vc = state.getChannel()) != null) {
				...
				int skipTime = Integer.parseInt(args[1]);
				
				int time = skipTime * 1000;
				player.getPlayingTrack().setPosition(player.getPlayingTrack().getPosition() + time);
				
				System.out.println(player.getPlayingTrack().getPosition());
				
				builder.setTitle("**I have skipped " + skipTime + " seconds**");
				builder.setDescription("I just skipped some loud earrape huh? (Currently a bit buggy lol)");
